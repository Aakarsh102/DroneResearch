...
class DecoderOnlyTrajectoryModel(nn.Module):
    ...
    def __init__(self, num_classes, locations, d_model=256, nhead=8, num_layers=6,
        ...
        self.transformer_layer = nn.TransformerEncoderLayer(d_model=d_model, nhead=nhead,dim_feedforward=4*nhead,
                                                            dropout=0.1)
    ...
    def _get_last_valid_position(self, past_positions, obs_masks, agent_masks):
        """
        Vectorized: returns the last observed position per agent.
        past_positions: (B, N, T_past, 2)
        obs_masks:       (B, N, T_past)   (1 if observed)
        agent_masks:     (B, N)           (1 if agent exists)
        """
        # Zero out unobserved times
        masked = past_positions * obs_masks.unsqueeze(-1)                     # (B,N,T,2)
        # Cumulative count over time to identify last obs index
        counts = obs_masks.long().cumsum(dim=2)                               # (B,N,T)
        last_idx = counts.argmax(dim=2)                                       # (B,N)

        # Gather the last positions
        idx = last_idx.view(*last_idx.shape, 1, 1).expand(-1, -1, 1, 2)       # (B,N,1,2)
        last_positions = masked.gather(2, idx).squeeze(2)                     # (B,N,2)

        # Zero out non-existent agents
        return last_positions * agent_masks.unsqueeze(-1)
    ...
    def _create_sequence_embeddings(self, batch, device):
        ...
        # Add future token to future positions
        # future_mask = torch.zeros(batch_size, num_agents, self.total_seq_len, device=device)
        # future_mask[:, :, self.T_past:] = 1
        # pos_emb = pos_emb + self.future_token * future_mask.unsqueeze(-1)

        # Apply agent masking
        obs = batch['obs_masks']                                              # (B,N,T_past)
        has_obs = obs.any(dim=2)                                               # (B,N)
        cumsum = obs.long().cumsum(dim=2)                                      # (B,N,T_past)
        last_obs = cumsum.argmax(dim=2)                                        # (B,N)

        # Gap = number of trailing unobserved timesteps
        raw_gap = (self.T_past - 1 - last_obs).clamp(min=0)                    # (B,N)
        gap = torch.where(has_obs, raw_gap, torch.full_like(raw_gap, self.T_past))

        # Embed and add
        enc = self.gap_embedding(gap)                                          # (B,N,d_model)
        enc = enc.unsqueeze(2).expand(-1, -1, self.T_past, -1)                 # (B,N,T_past,d_model)
        pos_emb = pos_emb + enc                                                # existing pos_emb
    ...
    def _forward_autoregressive(self, batch, device):
        ...
        for t in range(self.T_future):
            ...
            hidden_states = self.transformer_decoder(pos_emb_flat)
            # for layer in self.layers:
            #     hidden_states = layer(
            #         hidden_states,
            #         attention_mask=attention_mask_flat,
