...
class AlternatingTransformerEncoder(nn.Module):
    ...
    def forward(self, x, batch_size, num_agents, seq_len, attention_mask=None):
        ...
        for i, layer in enumerate(self.layers):
            if i % 2 == 0:
# THE INPUT ITSELF IS EXPECTED TO BE IN (B*N, T, d_model) FORMAT
                # Even layer: temporal attention (B*N, T, d_model)
                if current_x.shape[0] != batch_size * num_agents:
                ...
                if current_x.shape[0] != batch_size * seq_len:
        ...
        # Ensure output is in (B*N, T, d_model) format
...
class GraphInteractionModel(nn.Module):
    ...
    def _teacher_forcing_predictions(self, memory, attention_mask, current_pos_est,
        ...
        """Teacher forcing for training with corrected temporal encoding"""
        ...
        delta_predictions = self.delta_head(decoder_output)
        delta_mu = delta_predictions[..., :2]
        delta_var = self.var_activation(delta_predictions[..., 2:])
    ...
    def _generate_autoregressive_predictions(self, memory, attention_mask, current_pos_est,
                                           ...
                                           batch_size, num_agents, device):
        """Generate predictions autoregressively (similar structure to teacher forcing)"""
